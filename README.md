...
## 花データの内訳
- 野生の被子植物（species）: **4,909**
- 園芸系（著名な園芸交配種 = nothotaxa）: **91**

園芸枠は名前を1件ずつ列挙した固定リストで、iNaturalist の分類データに実在するものだけを採用する。
不足分を他の hybrid で埋めることはしない（リストのどれかが元データから消えたらビルドは失敗する）。
